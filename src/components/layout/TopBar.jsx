import { Bell, DownloadCloud, Mail, MessageCircle, Plus, Save, Search, Cloud } from 'lucide-react'
import Button from '../ui/Button.jsx'
export default function TopBar({ tab, search, setSearch, onNewInvoice, onSave, onPdf, onEmail, onWhatsapp, busy, dbStatus }) {
  ...
  return <>
    <div className="bar no-print"><div className="back">‹  ←</div><div className="searchbox"><Search size={16}/><input placeholder="Search..." value={search} onChange={e=>setSearch(e.target.value)} /><kbd>Ctrl K</kbd></div><div className={`cloudPill ${dbStatus === 'connected' ? 'online' : 'offline'}`}><Cloud size={15}/>{dbStatus}</div><button className="bell"><Bell size={18}/><span>3</span></button><div className="avatar">LD</div></div>
